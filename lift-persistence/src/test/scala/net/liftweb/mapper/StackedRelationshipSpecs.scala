...
class StackedRelationshipSpecs extends Specification {
  ...
  // H2 in memory by default. Set -Dmapper.test.provider=postgres to run the same
  // scenarios against a real server: save/delete_! chaining ends in INSERT/DELETE
  // and, for the cascade case, a rollback, which is where an embedded database and
  // a real one are most likely to differ.
  val provider =
    if (System.getProperty("mapper.test.provider") == "postgres") DbProviders.PostgreSqlProvider
    else DbProviders.H2MemoryProvider
...
class Teacher extends LongKeyedMapper[Teacher] with IdPK
  ...
  // A real column, not just the autogenerated key: an entity with no insertable
  // column produces `INSERT INTO teacher () VALUES ()`, which H2 accepts and
  // Postgres rejects, so a fixture without one cannot run on a real server.
